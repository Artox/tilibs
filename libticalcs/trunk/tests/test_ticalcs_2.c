...
static int cables_event_hook(CableHandle * handle, const CableEventData * event)
{
	int retval = 0;
	...
	{
		retval = old_ticables_event_hook(handle, event);
	}
	return retval;
}

static int calcs_event_hook(CalcHandle * handle, const CalcEventData * event)
{
	int retval = 0;
	if (NULL != handle && NULL != event)
	{
	...
	{
		retval = old_ticalcs_event_hook(handle, event);
	}
	return retval;
}
